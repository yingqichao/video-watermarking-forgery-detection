...
class ResBlock(nn.Module):
    def __init__(self, channel_in, channel_out,use_spectral_norm=False,init='xavier'):
        super(ResBlock, self).__init__()
        feature = 64
        self.conv1 = nn.Sequential(
            spectral_norm(nn.Conv2d(channel_in, feature, kernel_size=3, padding=1), use_spectral_norm),
            # nn.InstanceNorm2d(feature, track_running_stats=False),
            nn.ELU(inplace=True)
        )
        # self.relu1 = nn.LeakyReLU(negative_slope=0.2, inplace=True)
        self.conv2 = nn.Sequential(
            spectral_norm(nn.Conv2d(feature, feature, kernel_size=3, padding=1), use_spectral_norm),
            # nn.InstanceNorm2d(feature, track_running_stats=False),
            nn.ELU(inplace=True)
        )
...
class Inveritible_Decolorization_light_weight(nn.Module):
    def __init__(self, dims_in=[[3, 64, 64]], down_num=3, block_num=[2,2,2], subnet_constructor=ResBlock):
        super(Inveritible_Decolorization_light_weight, self).__init__()
        ...
        for i in range(down_num):
            ...
            current_dims[0][2] = current_dims[0][2] * 2
            for j in range(block_num[i]):
                b = RNVPCouplingBlock(current_dims, subnet_constructor=subnet_constructor, clamp=1.0) # subnet_constructor
                operations_Up.append(b)
...
class Inveritible_Decolorization_PAMI(nn.Module):
    def __init__(self, dims_in=[[3, 64, 64]], down_num=3, block_num=[8,8,8], subnet_constructor=ResBlock):
