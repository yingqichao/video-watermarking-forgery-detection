...
def test(model):
    ...
    sub_Imagenet = []
    for i in range(0,I_len,I_len//I_class_len):
    ...
    for (image, label) in sub_Imagenet:  
        ...
        target_label = torch.argmin(mypredict(model,image),1).item()
        print('target_label:',target_label)
        if opt.attack_method == 'FGSM' or opt.attack_method == 'IGSM':
            advpic = my_IGSM_T(model,image,target_label)
        else:
            advpic = image
        ...
        adv_target += (adv_label == target_label)
        ...
        total_target_list.append(template_list[:])
        ...
        for i in range(0,len(quality_list)):
            ...
            total_target_list[index][i] += (my_label == target_label)
        ...
        # adv + Iterative adversarial rounding
        if opt.compress_needlist == 1:
            ...
            for i in range(len(eta_list)):
                ...
                total_l2_with_pic_list.append(template_list[:])
                total_l2_with_adv_list.append(template_list[:])
                name_list.append('Iterative adversarial rounding_eta=%s'%eta_list[i]+' jpg adv') 

            for i in range(0,len(quality_list)):
                jpgadv_list = jpgcompress.iterativeAdversarialRounding(advpic,model,label,target_label,quality_list[i],eta_list)
                ...
                for jpgadv_i in range(len(jpgadv_list)):
                    ...
                    total_target_list[index+jpgadv_i+1][i] += (my_label == target_label)
        ...
        print('adv target:  %4f' %(adv_target*1.0/pic_num))
        ...
        for kind in range(0,index+1):
            for i in range(0,len(quality_list)):
                ...
                print(name_list[kind]+str(quality_list[i])+' attack success:  %.4f' %(total_target_list[kind][i]*1.0/pic_num))
                ...
                print()
        print('Finished '+str(pic_num)+'/'+str(len(sub_Imagenet)))
